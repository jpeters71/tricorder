...
class SX1509:
    ...
    def debounceConfig(self, configValue):
        ...
        self._writeByte(self.defs.RegDebounceConfig, configValue)


    def debounceEnable(self, pin):
        reg = self.defs.RegDebounceEnableB
        debounceEnable = self._readWordInt(addr=reg)
        log(f'debounce pin: {pin}, reg: {debounceEnable}')
        debounceEnable |= (1<<pin)
        self._write(addr=reg, val=debounceEnable)
    ...
    def _writePin(self, pin, highLow):
        tempRegDir = self._readWordInt(addr=self.defs.RegDirB)
        #print("_writePin. tempRegDir=", tempRegDir)

        log(f'writePin pin {pin}, highLow {highLow}, tempRegDir {tempRegDir}')
   
        if ((0xFFFF^tempRegDir)&(1<<pin)): #output mode
            tempRegData = self._readWordInt(self.defs.RegDataB)
            if (highLow):
                tempRegData |= (1<<pin)
            else:
                tempRegData &= ~(1<<pin)
            ...
            tempPullUp = self._readWordInt(self.defs.RegPullUpB)
            tempPullDown = self._readWordInt(self.defs.RegPullDownB)
            ...
            if highLow:
                ...
                tempPullUp &= ~(1 << pin)
                tempPullDown |= (1 << pin)
            self._writeWord(self.defs.RegPullUpB, tempPullUp)
            self._writeWord(self.defs.RegPullDownB, tempPullDown)
    ...
    def _readWordInt(self, addr):
        readValue = 0
        msb = 0
        lsb = 0
        self._i2c.writeto(self._addr, (0).to_bytes(1, "big"))
        rval = self._i2c.readfrom_mem(self._addr, addr, 2)
    ...
    def _readPin(self, pin):
        regDir = self.defs.RegDirB
        regData = self.defs.RegDataB
            
        tempRegDir = self._readWordInt(addr=regDir)
        log(f"readPin: {pin} tempRegDir={tempRegDir}")
        
        if (tempRegDir & (1<<pin)):  # If the pin is an input
            tempRegData = self._readWordInt(addr=regData)
            log(f"reg: {regData}, tempRegData: {tempRegData}")
    ...
    def digitalReadAllPins(self):
        return self._readWordInt(addr=self.defs.RegDataB)
