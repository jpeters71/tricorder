...
class SX1509:
 ...
 def __init__(self, i2c: I2C, address=0x3E, newSetup=True, CPUSpeed=0):
  self.i2c_device = i2c
  self._addr = address
 ...
 def pinMode(self, pin, inOut, HV_Mode=False):
  ...
  if ((inOut == PIN_TYPE_OUTPUT) or (inOut == PIN_TYPE_ANALOG_OUTPUT)):
   modeBit = 0
  else:
   modeBit = 1
  ...
  if (inOut == PIN_TYPE_ANALOG_OUTPUT):
 ...
 def setInputMode(self, pin, InputMode):
  ...
  if InputMode == PIN_TYPE_INPUT_PULLUP:
   tempRegData = self._read_reg_16(_SX1509_RegPullUpB)
   tempRegData |= (1<<pin)
   self._write_reg_16(_SX1509_RegPullUpB, tempRegData)
  elif InputMode == PIN_TYPE_INPUT_PULLDOWN:
   tempRegData = self._read_reg_16(_SX1509_RegPullDownB)
   tempRegData |= (1<<pin)
   self._write_reg_16(_SX1509_RegPullDownB, tempRegData)
  elif (InputMode == PIN_TYPE_INPUT_OPEN_DRAIN) or (InputMode == PIN_TYPE_ANALOG_OUTPUT):
 ...
 def _write_reg_8(self, reg, val):
  ...
  self.i2c_device.writeto_mem(self._addr, reg, val.to_bytes(1, "little"))
 ...
 def _write_reg_16(self, reg, val):
  ...
  lsb = (int(val) & 0xFF00)
  self.i2c_device.writeto_mem(self._addr, reg, lsb.to_bytes(1, "little"))
  self.i2c_device.writeto_mem(self._addr, reg, msb.to_bytes(1, "little"))

 def _read_reg_16(self, reg):
  rval = self._internal_read_16(reg)
  return ((rval[0]<<8) | rval[1])
 
 def _internal_read_16(self, reg):
  readValue = 0
  msb = 0
  lsb = 0
  self.i2c_device.writeto(self._addr, (0).to_bytes(1, "big"))
  rec1 = self.i2c_device.readfrom_mem(self._addr, reg, 2)
 ...
 def _read_reg_8(self, reg):
  val = bytes(0)        
  val = self._internal_read_16(reg)[0]
